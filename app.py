...
# ─── CONSTANTS ──────────────────────────────────────────────────────────────────
API_BASE_URL = "https://your-render-backend.onrender.com"   # ← swap with Aryan's URL
...
def init_state():
    defaults = {
        ...
        "dark_mode": False,
        "notifications": [
            {"id": 1, "msg": "Welcome to AlphaVerify!", "read": False, "time": "Just now", "icon": "🎉"},
        ],
...
def page_login():
    ...
    with c2:
        ...
        <div class="auth-card">
            <h2>🔬 AlphaVerify</h2>
            <p class="subtitle">Multi-Agent AI Talent Validation Suite</p>
        </div>
        """, unsafe_allow_html=True)

        with st.container():
            st.markdown("### Sign In")
            email = st.text_input("Email", placeholder="admin@alphaveify.ai", key="login_email")
            ...
            if login_btn:
                ...
                if email in db and db[email]["password"] == hash_pw(password):
                    ...
                    st.error("Invalid credentials. Try admin@alphaveify.ai / Admin@123")

            st.markdown("---")
            st.caption("Demo: `admin@alphaveify.ai` / `Admin@123`")
...
""" + "\n".join([f"{i+1}. {q}" for i, q in enumerate(iqs)])
    ...
    st.download_button(
        ...
        data=content,
        file_name=f"alphaveify_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.txt",
        mime="text/plain",
    )
